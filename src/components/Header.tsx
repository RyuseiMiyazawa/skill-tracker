...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 text-white shadow-2xl border-b border-purple-500/20">
        ...
        <div className="flex justify-between items-center">
          ...
          <nav className="flex gap-4 items-center">
            {(user || isGuest) && (
              <>
                {isGuest && (
                  <span className="hidden sm:inline-flex items-center rounded-full border border-amber-300/30 bg-amber-400/10 px-3 py-1 text-sm text-amber-100">
                    Guest Mode
                  </span>
                )}
                <Link
